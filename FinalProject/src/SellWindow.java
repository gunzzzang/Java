...
public class SellWindow extends JFrame {
    ...
    private String stockName;        // 매도할 종목
    private int tradePrice;          // 현재가(시장가)
    private JLabel holdingValue;     // 보유 수량/평단가

    public SellWindow(String stockName) {
        super(stockName + " 매도 창");
        ...
        SellCenterPanel centerPanel = new SellCenterPanel(stockName, tradePrice, gd); // 중앙에 보유정보/입력/버튼 패널
        add(new SellNorthPanel(), BorderLayout.NORTH); // 위쪽은 제목 패널
        add(centerPanel, BorderLayout.CENTER);         // 중앙 패널
        add(new SellSouthPanel(stockName, tradePrice, centerPanel), BorderLayout.SOUTH); // 매도하기 버튼 패널
    }

    // 상단 패널
    class SellNorthPanel extends JPanel {
        SellNorthPanel() {
            setBackground(new Color(246, 249, 252));
            setLayout(new BorderLayout());

            JLabel titleLabel = new JLabel("매도 창 (시장가)", SwingConstants.CENTER); // 텍스트를 가운데에 정렬
    ...
    // 중앙 패널
    class SellCenterPanel extends JPanel {
        ...
        SellCenterPanel(String stockName, int tradePrice, GameData gd) { // 생성자 : 종목명, 가격, GameData 정보를 받음
            // 필드에 저장
            ...
            holdingValue = new JLabel(
                    String.format("%d주 / %,d원", holdingQty, avgPrice),
            ...
            formPanel.add(quantityField); // 수량 입력칸 추가
            formPanel.add(new JLabel("예상 체결 금액"));
            formPanel.add(totalValueLabel); // 예상 금액 라벨 추가
            ...
            capitalLabel = new JLabel(
                    "보유 현금: " + String.format("%,d 원", gd.getCapital())
            ); // GameData에서 현재 현금을 가져와 라벨 생성
            ...
            // 금액 계산
            Runnable updateTotal = () -> { // 코드를 넘겨야하므로 Runnable을 씀
                try {
                    int q = Integer.parseInt(quantityField.getText()); // 수량 문자열을 정수로 변환
                    totalValueLabel.setText(
                            String.format("%,d 원", q * tradePrice)
                    ); // 수량 X 현재가 표시
                } catch (Exception e) {
            ...
            // quantityField 내용이 변할 때마다 updateTotal을 호출하도록 리스너 연결
            quantityField.getDocument().addDocumentListener(new SimpleDocListener(updateTotal));
            updateTotal.run();

            // 메소드를 이용해서 팔 수 있는 수량을 계산해서 입력칸에 넣고 총액 갱신
        ...
        int getQty() { // 매도버튼에서 수량을 가져갈 수 있게 반환
            return Integer.parseInt(quantityField.getText());
        }

        void refreshCapital() { // 현금이 매도 후 늘어날 때 라벨 최신 값으로 갱신
        ...
        private void setQtyByPercent(double percent, Runnable updateTotal) {
            ...
            int holdingQty = h.getQuantity(); // 보유 수량 기준
            int qty = (int) (holdingQty * percent); // 팔 수 있는 주 수 계산
            if (qty < 1) qty = 1;              // 최소 1주는 판다
            if (qty > holdingQty) qty = holdingQty;
    ...
    // 하단 영역
    class SellSouthPanel extends JPanel {

        SellSouthPanel(String stockName, int tradePrice, SellCenterPanel centerPanel) { // 생성자 : 종목명, 현재가, 중앙패널 정보를 받음
            setBackground(new Color(246, 249, 252));

            JButton sellButton = new JButton("매도하기");
            sellButton.setFont(new Font("맑은 고딕", Font.BOLD, 20));
            sellButton.setPreferredSize(new Dimension(250, 55));
            ...
            sellButton.addActionListener(e -> {
                try { // try-catch 구문 사용
                    int qty = centerPanel.getQty(); // 센터패널의 메소드 호출해서 사용자가 입력한 수량을 가져옴

                    if (!GameData.get().sell(stockName, qty, tradePrice)) { // 매도가 실패할 때
                    ...
                    centerPanel.refreshCapital(); // 매도 성공했으므로 보유 현금 최신 값으로 갱신
                    ...
                    holdingValue.setText(
                            String.format("%d주 / %,d원", newQty, newAvg)
                    ); // 화면에 최신 값 반영
    ...
    class SimpleDocListener implements DocumentListener {
        private Runnable r; // 텍스트가 바뀔때 실행할 코드를 저장하는 변수
        SimpleDocListener(Runnable r) { this.r = r; } // 입력이 바뀌면 r을 실행한다
        public void insertUpdate(javax.swing.event.DocumentEvent e) { r.run(); } // 텍스트가 추가될 때
        public void removeUpdate(javax.swing.event.DocumentEvent e) { r.run(); } // 텍스트가 지워질 때
        public void changedUpdate(javax.swing.event.DocumentEvent e) { r.run(); } // 다른 방식으로 바뀔 때
    }
}
